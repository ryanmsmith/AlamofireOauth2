...
class OAuth2Client: NSObject {
    ...
    private func postRequestHandler(data: NSData?, error: NSError?, token: ((accessToken: String?) -> Void)) -> Void {
        if let _ = error {
            print(error)
            ...
            if let data = data {
                do {
                    if let jsonResponse = try NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions.AllowFragments) as AnyObject? {
            ...
            if let optionalAuthCode = authorizationCode {
                ...
                Alamofire.request(.POST,
                    ...
                    .response { (request, response, data, error ) -> Void in
                        self.postRequestHandler(data, error: error, token: token)
                }
            }
        }
    ...
    func refreshToken(refreshToken: String, newToken: ((accessToken: String?) -> Void)) -> Void {
        ...
        Alamofire.request(.POST,
            ...
            encoding: Alamofire.ParameterEncoding.URL)
            .response { (request, response, data, error ) -> Void in
                self.postRequestHandler(data, error: error, token: newToken)
        }
    }
